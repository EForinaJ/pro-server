...
// Edit implements service.IManage.
func (s *sManage) Edit(ctx context.Context, req *dto_manage.Edit) (err error) {
	...
	_, err = tx.Model(dao.SysManage.Table()).WherePri(req.Id).
		Data(&entity).
		Update()
	if err != nil {
		return utils_error.Err(response.DB_SAVE_ERROR)
	...
	if len(req.Roles) > 0 {
		// 更新关联数据
		err = service.Role().AddRelated(ctx, tx, req.Id, req.Roles)
		if err != nil {
			return utils_error.Err(response.DB_SAVE_ERROR)
		}
		...
		_, err = tx.Model(dao.SysCasbin.Table()).
			...
			Where(dao.SysCasbin.Columns().V0, req.Id).Delete()
		if err != nil {
			return utils_error.Err(response.DB_SAVE_ERROR)
		}
		...
		for i, v := range roles {
			entites[i] = g.Map{
				...
				dao.SysCasbin.Columns().V0:    req.Id,
