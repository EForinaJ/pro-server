...
// GenToken 生成JWT
func GenFrontendToken(userId int64, username string) (string, error) {

	expire, err := g.Cfg().Get(gctx.New(), "jwt.frontend.expire")
	if err != nil {
		return "", err
	}
	...
	secret, err := g.Cfg().Get(gctx.New(), "jwt.frontend.secret")
	if err != nil {
		return "", err
	}
	//获得完整的、签名的令牌
...
func ParseFrontendToken(token string) (*CustomClaims, error) {
	secret, err := g.Cfg().Get(gctx.New(), "jwt.frontend.secret")
	if err != nil {
		return nil, err
	}
